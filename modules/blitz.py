...
# --- Control Commands (သားကြီး အမှာအတိုင်း .on / .off) ---
...
async def blitz_handler(client, message):
    ...
    # ၁။ Logic စစ်ဆေးခြင်း (OFF ထားရင်၊ အိပ်ချိန်ဆိုရင်၊ Cooldown မိနေရင် Skip မယ်)
    ...
    try:
        # ၃။ လူနဲ့တူအောင် ၂၀ စက္ကန့် စောင့်မယ် (သားကြီး ကိုယ်တိုင် ပြန်မလား ကြည့်ဖို့)
        ...
        if message.sticker:
            ...
            source = GREETINGS # Default
            if any(k in text for k in ["ဘာလုပ်", "doing", "လုပ်နေ"]): source = DOING_NOW
